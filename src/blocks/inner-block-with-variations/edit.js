...
const Edit = ( props ) => {
	...
	const backgroundClasses = classnames(
		`background ${ backgroundImage.repeat } ${ backgroundImage.attachment } ${ backgroundImage.customSize ? null : backgroundImage.backgroundSize } ${ backgroundImage.customSize ? null : backgroundImage.position } ${ backgroundImage.opacity }`,
	);

	const rowClasses = classnames(
		`row-container ${ borderStyle.style }`,
		{
	...
	const containerStyle = {
		'--klyra-box-shadow-color': boxShadow.color,
	}
	...
	if( ! variationName ) {
		return(
			<__experimentalBlockVariationPicker
				...
				instructions={ __( 'Select a variation', 'create-block-plugin' ) }
