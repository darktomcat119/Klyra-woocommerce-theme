	...
	return (
		<>
			...
			<InspectorControls>
				...
				/>
				<PanelBody
					title={ __( 'Dynamic block settings', 'create-block-plugin' ) }
					initialOpen
				>
					<ToggleControl
						label={ __( 'Show quote', 'create-block-plugin' ) }
						checked={ showQuote }
						onChange={ ( ) => setAttributes( { showQuote: ! showQuote } ) }
					/>
					<ToggleControl
						label={ __( 'Show customer', 'create-block-plugin' ) }
						checked={ showCustomer }
						onChange={ ( ) => setAttributes( { showCustomer: ! showCustomer } ) }
					/>
					<ToggleControl
						label={ __( 'Show image', 'create-block-plugin' ) }
						checked={ showImg }
